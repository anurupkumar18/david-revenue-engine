...
export function ContactTable({ contacts }: { contacts: ICPContact[] }) {
  if (!contacts.length) {
    return (
      <div className="panel grid place-items-center py-12 text-center">
        <p className="text-sm text-ink-dim">No contacts discovered yet.</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-4 rounded-[12px] border border-amber/25 bg-amber/[0.06] px-4 py-3 text-[12.5px] text-ink-dim">
        ...
        <table className="w-full text-left text-[13px]">
          <thead>
            <tr className="border-b border-line bg-surface-2/60">
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-ink-faint">Name</th>
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-ink-faint">Company</th>
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-ink-faint">Contact</th>
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-ink-faint">Verification</th>
            </tr>
          </thead>
          <tbody>
            {contacts.map((c) => (
