...
export function LandAndExpandPlanView({ plan }: { plan: LandAndExpandPlan }) {
  return (
    <div className="panel-2 p-4">
      <div className="flex items-stretch gap-1.5">
        <Node label="Land" path={plan.landOffer} />
        <ArrowRight size={15} className="mt-5 shrink-0 text-ink-faint" />
        <Node label="Expand" path={plan.expansionOffer} />
        {plan.longTermOffer && (
          <>
            <ArrowRight size={15} className="mt-5 shrink-0 text-ink-faint" />
            <Node label="Long-term" path={plan.longTermOffer} />
          </>
        )}
      </div>
      ...
      <div className="mt-4 flex items-start gap-2 border-t border-line pt-3">
        ...
        <p className="text-[12.5px] leading-snug text-ink-dim">{plan.revenueLogic}</p>
      </div>
    </div>
  );
}
